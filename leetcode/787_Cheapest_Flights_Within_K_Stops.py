        ...
        for edge in range(1, k + 2):
            for dst_node in range(n):
                for src_node, price in graph[dst_node]:
                    price_table[edge][dst_node] = min(
                        price_table[edge][dst_node],
                        price_table[edge - 1][src_node] + price,
                    )
...
class SolutionTimeLimit:
    def findCheapestPrice(
        ...
        def dfs(node: int, weight: int, count: int):
            nonlocal result
            if node in visited:
                return
            if node == dst:
